...
class RobertaTrainer:
            ...
            for phase in ['train', 'valid']:
                ...
                for i, batch in enumerate(tqdm(self.dataloaders[phase], total=len(self.dataloaders[phase]), desc=f"{phase}..")):
                    ...
                    if phase == "train":
                        ...
                        loss = self._training_step(model_inputs, labels)
                        ...
                        loss = self._validation_step(model_inputs, labels)

                    if i % self.config.log_step == 0:
                        self.logger.info(f"{'Epoch':<15}{epoch + 1}\n{'Phase':<15}{phase}\n{'Step':<15}{step}\n{'Total Loss':<15}{loss:.4f}\n")
                        if phase == "train":
                            # step / loss
                            train_loss_history.append([step, loss])
                        else:
                            valid_loss_history.append([step, loss])
                    epoch_loss += loss * batch['input_ids'].size(0)
    ...
    def _validation_step(self, batch: Dict, labels: Dict):
        output = self.model(**batch)
        # Losses
        loss = self.mlm_criterion(output['logits'].view(-1, output['logits'].size(-1)), labels['mask_label'].view(-1)).detach()
